...
- **在APP工程中的`tlib_usr.c`中完成库函数定义**
  - ![](./pic/add_to_usr.png)

- **在内核工程的`tlib.c` 的`syscall_kernel_knl` 和 `syscall_kernel` 中 添加`case 条件` 并按照要求实现`syscall`**
...
- **IO_CONTRL调用规范**
  ...
  - **以串口为例 上图所示**
  - **可能需要注意的就是可以挂的中断处理程序的钩子函数**
  - **注意在Kernel工程有而加入相应的配置代码！**