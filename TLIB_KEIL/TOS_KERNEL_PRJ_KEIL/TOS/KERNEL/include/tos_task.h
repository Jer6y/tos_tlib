...
struct k_task_st {
    k_stack_t          *sp;                     /**< task stack pointer. This lady always comes first, we count on her in port_s.S for context switch. */
		...
		k_stack_t 				 *k_sp;										/**< kernel task stack pointer. This lady always comes second, we count on her in port_s.S for context switch. */
		
		uint8_t							in_kernel;							/**< state for is in_kernel> */
		
		uint8_t							stat_swtch_pending;			/**< pending register for state switch in kernel and user> */
    ...
    k_stack_t          *stk_base;               /**< task stack base address */
		...
		k_stack_t 				 *k_stk_base;							/**< kernel task stack base address */
		
		size_t							k_stk_size;							/**< kernel stack size of the task */
#endif
