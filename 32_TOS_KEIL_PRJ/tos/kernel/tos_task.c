...
#if TOS_CFG_TLIB_EN >0u && TOS_CFG_OBJ_DYNAMIC_CREATE_EN >0u
		pid_t pid = find_set_pid(task);
		if(pid == PID_FULL) {
			return K_ERR_TLIB_PID_FULL;
		}
#endif
...
__STATIC__ void task_free(k_task_t *task)
{
    ...
    tos_mmheap_free(task);
		...
		if(task->k_stk_base!=K_NULL)
		{
			tos_mmheap_free(task->k_stk_base);
			task->k_stk_base = K_NULL;
		}
#endif
}
...
__API__ k_err_t tos_task_create_dyn_tlib(
                                                    const char *name,
                                                    k_task_entry_t entry,
                                                    void *arg,
                                                    k_prio_t prio,
																										...
																										pid_t* pid_ad)
{
    ...
    if (!stk_base) {
		...
		k_stk_base = tos_mmheap_alloc(k_stk_size);
		if (!stk_base) {
    ...
    err = tos_task_create_tlib(the_task,name,entry,arg,prio,stk_base,stk_size,k_stk_base,k_stk_size,timeslice,pid_ad);
    if (err != K_ERR_NONE) {
        task_free(the_task);
				TOS_CPU_INT_ENABLE();
				return err;
...
__API__ k_err_t tos_task_destroy_dyn_tlib(pid_t pid)
{
		if(pid == 0xffffffff)
		return tos_task_destroy_dyn(k_curr_task);
		k_task_t* tcb = pid_to_tcb(pid);
		if(tcb ==NULL)
			return K_ERR_TLIB_INVALID_PID;
		return tos_task_destroy_dyn(tcb);		
}
