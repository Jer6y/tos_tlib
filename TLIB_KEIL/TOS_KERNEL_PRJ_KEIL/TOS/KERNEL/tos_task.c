...
__STATIC__ void task_free(k_task_t *task)
{
		...
		if(task->k_sp != K_NULL)
		{
			tos_mmblk_free(&k_mmblk_kernel_stk_ctl,(void*)task->k_stk_base);
			task->k_sp = K_NULL;
		}
#endif
...
__API__ k_err_t tos_task_create_tlib(				k_task_t *task,
                                            const char *name,
                                            k_task_entry_t entry,
                                            void *arg,
                                            k_prio_t prio,
                                            ...
                                            k_timeslice_t timeslice)
{
    ...
    task->sp        = cpu_task_stk_init((void *)entry, arg, (void *)task_exit, stk_base, stk_size);
    task->k_sp			= k_stk_base + k_stk_size;
		task->entry     = entry;
    task->arg       = arg;
    task->prio      = prio;
		task->k_stk_base = k_stk_base;
		task->k_stk_size = k_stk_size;
    task->stk_base  = stk_base;
    task->stk_size  = stk_size;
		task->in_kernel  = 0;
		task->stat_swtch_pending =0;
...
__API__ k_err_t tos_task_destroy_tlib(k_task_t* task)
{
		TOS_IN_IRQ_CHECK();
    if (unlikely(!task)) {
        task = k_curr_task;
    ...
    if (!knl_object_alloc_is_static(&task->knl_obj)) {
		...
		if(task->k_sp != K_NULL)
		{
			tos_mmblk_free(&k_mmblk_kernel_stk_ctl,task->k_stk_base);
			task->k_sp= K_NULL;
		}
...
#if TOS_CFG_OBJ_DYNAMIC_CREATE_EN >0u
__API__ k_err_t tos_task_create_dyn_tlib( k_task_t** task,
																					const char *name,
																					k_task_entry_t entry,
																					void *arg,
																					k_prio_t prio,
																					...
																					k_timeslice_t timeslice)
{
    ...
    if (!stk_base) {
		...
		TOS_CPU_INT_DISABLE();
    the_task->stk_base   = stk_base;
		the_task->k_stk_base = k_stk_base;
    err = tos_task_create_tlib(the_task,name,entry,arg,prio,stk_base,stk_size,k_stk_base,k_stk_size,timeslice);
    if (err != K_ERR_NONE) {
        task_free(the_task);
				TOS_CPU_INT_ENABLE();
				return err;
...
__API__ k_err_t tos_task_destroy_dyn_tlib(k_task_t* task)
{
    k_err_t err;
    TOS_IN_IRQ_CHECK();
    if (unlikely(!task)) {
...
__API__ k_err_t tos_task_destroy_dyn_stk_chk(k_task_t* task)
{
		k_err_t err;
...
__API__ k_err_t tos_task_destroy_stk_chk(k_task_t* task)
{
	return task_do_destroy(task);
}
